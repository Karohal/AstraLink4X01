...
**Primary Dependencies**: Unity 6000.3.9f1 ; Universal Render Pipeline 17.3.0 (com.unity.render-pipelines.universal) ; Input System 1.18.0 (com.unity.inputsystem) ; AI Navigation 2.0.10 (com.unity.ai.navigation) ; Unity Test Framework 1.6.0 (com.unity.test-framework) ; UI Toolkit (com.unity.modules.uielements, déjà présent) ; sérialisation JSON — bibliothèque exacte à trancher (cf. research.md)

**Storage**: fichiers de sauvegarde locaux (JSON) dans le dossier de données persistantes de l'application ; aucune base de données, aucun serveur (cf. research.md pour le choix de la bibliothèque de sérialisation)
...
| III. Conventions de code et structure cohérentes | Namespaces `Game.<Module>` et dossiers `Assets/_Project/Scripts/<Module>/` respectés. La liste de modules de la Constitution (`Economy, Building, Procedural, Core`) est étendue avec de nouveaux modules requis par le périmètre du spec (`Colonists, Research, Logistics, Civilization`) — la Constitution donne cette liste à titre d'exemple ("ex:") pour le namespace, mais la qualifie de « fixée » pour les dossiers. Extension documentée et justifiée en Complexity Tracking ci-dessous. | PASS AVEC NOTE (cf. Complexity Tracking) |
